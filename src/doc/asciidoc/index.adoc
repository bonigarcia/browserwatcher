...
* `window.postMessage({ type: "stopRecording" }, "*");` : For stopping the recording and download the webm file from the browser.
* `window.postMessage({ type: "stopRecordingBase64" }, "*");` : For stopping the recording and get the recording encoded as Base64 in the response.
